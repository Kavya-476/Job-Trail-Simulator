...
import { ArrowLeft, Clock, CheckCircle, Lock, Play, ChevronRight, Lightbulb, XCircle, BarChart2, Shield, Puzzle, Layout, RotateCcw, Zap } from 'lucide-react';
    ...
    const levelTasks = {
        'Beginner': [
            {
                ...
                description: "Identify and resolve logic errors in a legacy JavaScript utility.",
                status: 'COMPLETED',
                ...
                feedback: {
                    title: "Fix the Broken Function",
                    score: 100,
                    strengths: ["Proper use of Docker volumes", "Clean environmental variable configuration"],
                    weaknesses: ["None identified - Perfect execution"],
                    tip: "Maintain this level of documentation for complex setups."
                }
            },
            {
                id: 2,
                title: "Reverse Engineer Algorithm",
                description: "Analyze and explain the winning logic of a Tic-Tac-Toe implementation.",
                status: 'COMPLETED',
                icon: <CheckCircle className="w-6 h-6" />,
                iconBg: "bg-green-100 dark:bg-green-500/10 text-green-600 dark:text-green-400",
                duration: "10m",
                feedback: {
                    ...
                    score: 92,
                    strengths: ["Clear logic decomposition", "Identified edge cases in diagonals"],
                    weaknesses: ["Could be more concise in the row checking explanation"],
                    tip: "Try using mathematical patterns to describe geometric logic."
                }
            },
            {
                id: 3,
                title: "Sanitize User Inputs",
                description: "Secure a form against common script injections and validation errors.",
                status: 'NEXT TASK',
                icon: <Shield className="w-6 h-6" />,
                iconBg: "bg-blue-100 dark:bg-blue-500/10 text-[#2563EB]",
                duration: "15m"
            },
            {
                id: 4,
                title: "Optimize Search Algorithm",
                description: "Refactor slow iteration logic into a high-performance Hash Map lookup.",
                status: 'LOCKED',
                icon: <Zap className="w-6 h-6" />,
                iconBg: "bg-slate-100 dark:bg-navy text-slate-400",
                duration: "20m"
            },
            {
                id: 5,
                title: "API Integration Fix",
                description: "Finalize the connection between local state and the legacy backend API.",
                status: 'LOCKED',
        ...
        'Intermediate': [
            /* {
                ...
                description: "Refactor complex queries to reduce latency and improve database indexing strategy.",
                status: 'COMPLETED',
                icon: <CheckCircle className="w-6 h-6" />,
                iconBg: "bg-green-100 dark:bg-green-500/10 text-green-600 dark:text-green-400",
                duration: "40m",
                ...
                feedback: {
                    title: "Database Optimization",
                    score: 95,
                    strengths: ["Excellent indexing strategy", "Optimized nested joins"],
                    weaknesses: ["Could use more descriptive alias names"],
                    tip: "Always check the execution plan before finalizing queries."
                }
            }, */
            {
                id: 5,
                title: "Add Feature to To-Do List",
                description: "Implement persistent storage and filter functionality for the existing task management system.",
                status: 'IN PROGRESS',
                icon: <Layout className="w-6 h-6" />,
                iconBg: "bg-blue-100 dark:bg-blue-500/10 text-blue-600 dark:text-blue-400",
                duration: "45m",
                tags: ["FRONTEND + BACKEND", "STATE MANAGEMENT"]
            },
            {
                id: 6,
                title: "Build RESTful API Endpoint",
                description: "Create a secure POST endpoint with input validation and proper HTTP response codes.",
                status: 'NOT STARTED',
                icon: <Puzzle className="w-6 h-6" />,
                iconBg: "bg-slate-100 dark:bg-navy-light text-slate-400",
                duration: "60m",
        ...
        'Professional': [
            {
                ...
                description: "Create a multi-region architecture for a real-time messaging platform supporting 10M+ daily active users.",
                status: 'NEXT TASK',
                icon: <Puzzle className="w-6 h-6" />,
                iconBg: "bg-blue-100 dark:bg-blue-500/10 text-[#2563EB]",
                duration: "90m Estimated",
                tags: ["HIGH LOAD", "INFRA"]
            }
        ]
    };

    const currentTasks = levelTasks[normalizedLevel] || levelTasks['Beginner'];
        ...
        <div className="animate-in fade-in duration-700">
            <div className="max-w-6xl mx-auto py-10 space-y-10">
                ...
                <div className="flex flex-col md:flex-row justify-between items-start gap-8">
                    ...
                    <button className="flex items-center gap-2 px-5 py-2.5 bg-white dark:bg-navy-light border border-slate-200 dark:border-slate-800 rounded-xl shadow-sm hover:shadow-md transition-all text-sm font-bold text-slate-700 dark:text-slate-200">
                ...
                <div className="space-y-6">
                    ...
                    <div className="space-y-4">
                        {/* Logic to render tasks based on level - Showing only first 3 */}
                        {currentTasks.slice(0, 3).map((task) => (
                            <div key={task.id} className={`p-6 rounded-2xl border flex flex-col md:flex-row items-start md:items-center justify-between gap-6 transition-all ${getStatusStyles(task.status)}`}>
                                ...
                                <div className="flex flex-col items-end gap-3 w-full md:w-auto">
                                    <div className={`flex items-center gap-1.5 text-[10px] font-bold ${task.status === 'Professional' ? 'text-blue-500' : 'text-slate-400 dark:text-slate-500'}`}>
                                    ...
                                    <div className="flex flex-col sm:flex-row gap-2 w-full md:w-auto">
                                        {task.status === 'COMPLETED' ? (
                                            <>
                                                ...
                                                <Link
                                                    to={task.id === 2 ? `/simulation/${job.id}/${normalizedLevel}/explanation` : task.id === 3 ? `/simulation/${job.id}/${normalizedLevel}/sanitize` : task.id === 5 ? `/simulation/${job.id}/${normalizedLevel}/todo-feature` : task.id === 6 ? `/simulation/${job.id}/${normalizedLevel}/rest-api` : task.id === 7 ? `/simulation/${job.id}/${normalizedLevel}/system-design` : `/simulation/${job.id}/${normalizedLevel}`}
